...
GLubyte *TextureLoader::loadAlphaTGA ( const char *tga_file_name, int _height,
                                      int _width )
{
    GLubyte *full_texture = loadTGA( std::strcat((char*)bundlePath(), tga_file_name), _height, _width);
...
{
#ifndef _WIN32
    std::ifstream file(tga_file_name);
#else
    
    ifstream file(tga_file_name, ios::binary);
#endif
    if (file.fail()) {
        std::cerr << "Error opening texture file '" << tga_file_name << "'." << std::endl;
        exit(1);
    }
    ...
    if (memcmp(static_header, header_image, sizeof(static_header)) != 0) {
        std::cerr << "Texture file '" << tga_file_name << "' is not in uncompressed "
        "TGA format." << std::endl;
        exit(1);
    }
    ...
    if (width != _width || height != _height) {
        std::cerr << "Texture file '" << tga_file_name << "' does not have the expected "
        "height and width.  [" << height << 'x' << width << "] vs expected ["
        << _height << 'x' << _width << "]." << std::endl;
        exit(1);
    }
    
    int color_depth = header[4];
    if (color_depth != 24 && color_depth != 32) {
        std::cerr << "Texture file '" << tga_file_name << "' has an unsupported color "
        "depth." << std::endl;
        exit(1);
    }
    if (_color_depth != 0 && _color_depth != color_depth) {
        std::cerr << "Texture file '" << tga_file_name << "' does not have the expected "
        "color depth." << std::endl;
        exit(1);
    }
...
{
#ifndef _WIN32
    std::ofstream file(tga_file_name);
#else
    ofstream file(tga_file_name, ios::binary);
#endif
    if (file.fail()) {
        std::cerr << "Error creating texture file '" << tga_file_name << "'." << std::endl;
        exit(1);
    }
