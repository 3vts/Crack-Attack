...
void Displayer::rerankScoreRecord (   )
{
    ...
    char rank_string[8];
    std::ostrstream s(rank_string, 8);
    for (int n = Score::player_rank; n--; ) {
        std::ostrstream s(rank_string, 8);
        s << (GC_SCORE_REC_LENGTH - n) << std::ends;
                                          ...
                                          const char *name, GLubyte *texture )
{
    ...
    std::ostrstream s(score_string, 32);
    s << score << std::ends;
    
    char rank_string[8];
    std::ostrstream t(rank_string, 8);
    t << (GC_SCORE_REC_LENGTH - rank) << std::ends;
...
void Displayer::generateScoreRecord (   )
{
    ...
    char score[32];
    std::ostrstream s(score, 32);
    s << Score::record[GC_SCORE_REC_LENGTH - 1].score << std::ends;
