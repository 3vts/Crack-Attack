...
void Score::writeScoreRecord (   )
{
    ...
    if (file.fail()) {
        std::cerr << "Error writing to score record file '" << file_name << "'." << std::endl;
        exit(1);
    }
...
{
    std::ifstream file(GC_DEFAULT_REC_FILE_NAME);
    if (file.fail()) {
        std::cerr << "Error opening data file '" GC_DEFAULT_REC_FILE_NAME "'."
        << std::endl;
        exit(1);
    }
