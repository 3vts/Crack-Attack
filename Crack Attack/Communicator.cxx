    ...
    switch (mode & (CM_SERVER | CM_CLIENT)) {
        case CM_SERVER: {
            ...
            if (bind(connection_socket, (sockaddr *) &address, sizeof(address)) < 0) {
                std::cerr << "Port " << port << " is busy." << std::endl;
                exit(1);
            }
            ...
            std::cout << "Connection made by " << inet_ntoa(address.sin_addr) << '.' << std::endl;
            break;
            ...
            if (!host) {
                std::cerr << "Host '" << host_name << "' not found." << std::endl;
                exit(1);
            }
            ...
            if (ntohl(server_version_id) != version_id) {
                std::cerr << "Connected to incompatible version." << std::endl;
                exit(1);
            }
            ...
            std::cout << "Connection made to " << inet_ntoa(address.sin_addr) << ':'
            << (short) port << '.' << std::endl;
            break;
        } }
